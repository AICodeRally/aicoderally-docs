...
for (const comment of comments) {
  comment.thread = await prisma.thread.findUnique({ where: { id: comment.threadId } });
  comment.reactions = await prisma.reaction.findMany({ where: { commentId: comment.id } });
}
```
...
```typescript
// ✅ Good: Use Prisma query builder (parameterized)
const user = await prisma.user.findUnique({
...
await prisma.$queryRaw(query);  // Never do this!
```
...
const comments = await prisma.comment.findMany({
  ...
  take: 50,  // First 50 results
});

// ❌ Bad: Fetch everything
const comments = await prisma.comment.findMany();  // Could be millions of rows!
```
