...
```
Does this solve ONE specific problem? (e.g., "track time")
├─ YES → It's a Module
│
NO ↓
└─ Does it combine multiple capabilities?
   ├─ YES, in a specific way → It's an App (or Studio App)
   │                          Build in: apps/studio/
   │
   └─ NO → It's a Library or Component
```
...
Performance Dashboard =
  ...
  + reporting-engine (export)
```

### Pattern 2: Conditional Composition

```
Feature = Module A + (if tier == premium, Module B)

Example:
Basic Project Tracker = project-tracker
Advanced Version = project-tracker + ai-assistant
```

### Pattern 3: Deep Composition

```
Complex Feature = Module A
  └─ uses Module B
    └─ uses Module C
      └─ uses Module D

Example:
Accounting Dashboard = accounting-module
  ├─ uses: budget-management
